...
보통 UI가 존재하는 TodoApp만 만들다보니 CLI로 TodoApp을 만든 경험은 이번이 처음이다. 기본적으로 로직자체에서 큰 차이는 없으나 UI가 가진 편리함을 CLI로 표현해야하다보니 중간 절차를 모두 표현해줘야하는 번거로움이 있었다. 또한 Todo 안에 여러가지 속성을 추가하다보니 이에 대한 유효성검사를 해줘야한다는 귀찮음이 존재했다.(사실 이 부분은 넘어간 부분들이 많이 존재한다. 😅)  타입스크립트를 사용하다보니 타입 체크에 대한 오류를 많이 경험하게 되었고 이를 해결하는 과정에서 타입스크립트와 많이 익숙해지는 좋은 시간이 되었다. 뿐만 아니라 어떤 방식으로 타입스크립트를 타입스크립트 답게 사용하면 될지에 대해서도 조금은 알 수 있는 시간이였다. 다만, 클래스 기반의 앱을 만들다보니 클래스에 익숙하지 않은 부분에 대한 의문점들이 남게 되었는데, 이 부분은 차차 알아나가는 과정을 가져야할 듯한다.(ex. edit 기능)
</details>
